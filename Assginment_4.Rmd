...
Dataset contains `nrow(train)` rows and `ncol(train)` variables. There are 6 missing data in Age, 454 missing data of YOK, 445 missing data of income, 464 missing data of home value. We will use mean of the colums to fill in any missing values. We will also tranform money and other fields numerical fields. 
...
### Model for amount
```{r}
t<- na.omit(train_clean)
full.model <- lm(TARGET_AMT ~ .-TARGET_FLAG , train_clean, na.action = na.exclude)
summary(full.model)
...
full.model.glm.step <- step( full.model.glm,trace = 0, keep = NULL)

  
summary(full.model.step) 
full.model.step$anov
summary(full.model.glm.step) 
```
...
train_flag <- train_clean[,-c(2)] 
#full.model.glm <- glm(TARGET_FLAG ~. , data = train_flag, family = binomial(lcofnk='logit'))
#summary(full.model.glm)

b <- regsubsets(TARGET_AMT ~.-TARGET_FLAG,data = t ,nvmax = 27)
rs <- summary(b)
rs$which

# b <- regsubsets(TARGET_FLAG ~.,data = train_flag)
#rs <- summary(b)
#rs$which
