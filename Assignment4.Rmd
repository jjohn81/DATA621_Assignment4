...
full.model.glm <- glm(TARGET_FLAG ~. , data = train_flag, family = binomial(link='logit'))
...
transform_model.glm <- glm(TARGET_FLAG ~., data = train_clean, family = binomial(link='logit'))
...
#####Full Model 
First model includes all variables excluding index, this model has Adjusted R-squared of  0.2886. A model with out target falg has low Adjusted R-squared of   0.06665.    Model with TARGET_FLAG  has fewer statistically significatint predictor. 
```{r}

full.model <- lm(TARGET_AMT ~ .  , train, na.action = na.exclude)
summary(full.model)

train1 <- train[ which(train$TARGET_FLAG==1),] 
train1 <- train1[,-c(1,2)] 

full.model.onlyFlag <- lm(TARGET_AMT ~ .  ,data= train1, na.action = na.exclude)
summary(full.model.onlyFlag)
...
#####Step 
```{r}
full.model.step <- step(full.model, trace = 0, keep = NULL)
summary(full.model.step) 
full.model.step$anov
```
...
# Train the model
model <- train(TARGET_AMT ~., data = train, method = "lm",
...
```{r}
train_flag$predict_full_glm <- predict(full.model.glm, train_flag, type='response')

roc_reduced_model <- roc(train_flag$TARGET_FLAG, train_flag$predict_full_glm, plot=T, asp=NA,
